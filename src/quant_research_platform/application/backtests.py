...
def _audit_fill_costs(fill: object, context: _AuditContext) -> None:
    ...
    session = _date_value(_field(fill, ("session", "execution_session")), "fill.session")
    quantity = _quantity(_field(fill, ("quantity", "amount")), "fill.quantity")
    if quantity == 0:
        raise _BacktestFailure(
            (_error("A fill quantity was zero.", field_path="fills.quantity", session=session, symbol=symbol),)
        )
    base = _decimal(_field(fill, ("base_adjusted_open", "base_open")), "fill.base_adjusted_open")
    price = _decimal(_field(fill, ("fill_price", "price")), "fill.fill_price")
    gross = _decimal(_field(fill, ("gross_notional", "notional")), "fill.gross_notional")
    ...
    if base <= 0 or price <= 0:
        raise _BacktestFailure(
            (_error("A fill used a non-positive price.", field_path="fills.price", session=session, symbol=symbol),)
        )
    ...
    if expected_price <= 0 or price != expected_price:
        raise _BacktestFailure(
            (_error("A fill price does not equal the configured adverse-slippage formula.", field_path="fills.fill_price", session=session, symbol=symbol),)
        )
    expected_gross = quantize_money(abs(quantity) * price)
    if gross != expected_gross:
        raise _BacktestFailure(
            (_error("A fill gross notional does not equal actual whole-share fill notional.", field_path="fills.gross_notional", session=session, symbol=symbol),)
        )
    expected_commission = quantize_money(expected_gross * context.commission_rate)
    if commission != expected_commission:
        raise _BacktestFailure(
            (_error("A fill commission does not equal the configured cost formula on actual notional.", field_path="fills.commission", session=session, symbol=symbol),)
        )
    expected_slippage = quantize_money(abs(price - base) * abs(quantity))
    if slippage_cost != expected_slippage:
        raise _BacktestFailure(
            (_error("A fill slippage cost does not equal adverse price difference times quantity.", field_path="fills.slippage_cost", session=session, symbol=symbol),)
        )
...
def _audit_position(position: object, *, session: date | None) -> Decimal:
    ...
    if quantity <= 0:
        raise _BacktestFailure(
            (_error("A position quantity was not a positive whole share count.", field_path="portfolio.positions.quantity", session=session, symbol=symbol),)
        )
    mark_price = _decimal(_field(position, ("mark_price", "price", "last_sale_price")), "position.mark_price")
    market_value = _decimal(_field(position, ("market_value", "value")), "position.market_value")
    if mark_price <= 0:
        raise _BacktestFailure(
            (_error("A position mark price was not positive.", field_path="portfolio.positions.mark_price", session=session, symbol=symbol),)
        )
    expected_value = quantize_money(Decimal(quantity) * quantize_money(mark_price))
    if market_value != expected_value:
        raise _BacktestFailure(
            (_error("A position market value does not reconcile to quantity times mark price.", field_path="portfolio.positions.market_value", session=session, symbol=symbol),)
        )
    return market_value
...
def _audit_state(state: object, context: _AuditContext) -> None:
    ...
    gross = _decimal(_field(state, ("gross_exposure", "gross_position_value", "gross")), "portfolio.gross_exposure")
    equity = _decimal(_field(state, ("portfolio_equity", "equity", "portfolio_value")), "portfolio.portfolio_equity")
    leverage = _decimal(_field(state, "leverage"), "portfolio.leverage")
    if cash < 0:
        raise _BacktestFailure(
            (_error("Portfolio cash became negative.", field_path="portfolio.cash_balance", session=session),)
        )
    ...
    for position in positions:
        ...
        if symbol in symbols:
            raise _BacktestFailure(
                (_error("A portfolio state contained duplicate position symbols.", field_path="portfolio.positions", session=session, symbol=symbol),)
            )
        symbols.add(symbol)
    ...
    if gross < 0 or gross != calculated_gross:
        raise _BacktestFailure(
            (_error("Gross exposure does not equal the sum of marked position values.", field_path="portfolio.gross_exposure", session=session),)
        )
    reconciled_equity = quantize_money(cash + gross)
    if abs(equity - reconciled_equity) > Decimal("0.01"):
        raise _BacktestFailure(
            (_error("Portfolio equity does not reconcile to cash plus marked positions.", field_path="portfolio.portfolio_equity", session=session),)
        )
    if equity <= 0:
        raise _BacktestFailure(
            (_error("Portfolio equity was not positive.", field_path="portfolio.portfolio_equity", session=session),)
        )
    expected_leverage = gross / equity
    if leverage < 0 or leverage > Decimal("1") or abs(leverage - expected_leverage) > Decimal("0.000000000001"):
        raise _BacktestFailure(
            (_error("Portfolio gross leverage was outside the inclusive [0, 1] bound or did not reconcile.", field_path="portfolio.leverage", session=session),)
        )


def _audit_fill_transitions(
    states: Sequence[object], fills: Sequence[object]
) -> None:
    ...
    for state in states:
        ...
        previous_positions_value = _field(
            previous_state, ("positions", "holdings"), ()
        )
        ...
        for fill in session_fills:
            ...
            expected_cash += (
                gross - commission if quantity < 0 else -gross - commission
            )
...
def _audit_generic_events(output: object, states: Sequence[object]) -> None:
    ...
    for collection_name in ("actions", "corporate_actions", "marks", "valuation_marks"):
        ...
        if isinstance(collection, (str, bytes, bytearray)):
            raise _BacktestFailure(
                (_error("The ledger event collection has an invalid shape.", field_path=collection_name),)
            )
        try:
            events = tuple(collection)
        except TypeError as error:
            raise _BacktestFailure(
                (_error("The ledger event collection is not iterable.", field_path=collection_name),)
            ) from error
        ...
        for event in events:
            ...
            if event_session is None or event_session not in state_sessions:
                raise _BacktestFailure(
                    (_error("A ledger action or mark has no corresponding portfolio session.", field_path=f"{collection_name}.session", session=event_session),)
                )
            if previous is not None and event_session < previous:
                raise _BacktestFailure(
                    (_error("Ledger actions or marks are out of chronological order.", field_path=collection_name, session=event_session),)
                )
    ...
    *,
    ...
    try:
        ...
        if any(not hasattr(output, name) for name in required):
            raise _BacktestFailure(
                (_error("The engine output omitted a required audited role.", field_path="core_output"),)
            )
        ...
        if not states:
            raise _BacktestFailure(
                (_error("The engine output contained no portfolio marks.", field_path="portfolio_states"),)
            )
        first_state_session = _date_value(_field(states[0], "session"), "portfolio.session")
        if _decimal(_field(states[0], ("cash_balance", "cash")), "portfolio.cash_balance") != context.initial_equity:
            raise _BacktestFailure(
                (_error("The first portfolio state did not start with USD 100000 cash.", field_path="portfolio.initial_cash", session=first_state_session),)
            )
        previous_session: date | None = None
        for state in states:
            session = _date_value(_field(state, "session"), "portfolio.session")
            if session is None or (previous_session is not None and session <= previous_session):
                raise _BacktestFailure(
                    (_error("Portfolio marks are not strictly chronological.", field_path="portfolio_states.session", session=session),)
                )
            _audit_state(state, context)
            previous_session = session

        if evaluation_range is not None:
            if first_state_session is None or first_state_session < evaluation_range.start:
                raise _BacktestFailure(
                    (_error("Portfolio output begins before the requested evaluation range.", field_path="evaluation_range.start", session=first_state_session),)
                )
            last_state_session = _date_value(_field(states[-1], "session"), "portfolio.session")
            if last_state_session is None or last_state_session > evaluation_range.end:
                raise _BacktestFailure(
                    (_error("Portfolio output extends beyond the requested evaluation range.", field_path="evaluation_range.end", session=last_state_session),)
                )
        ...
        for order in orders:
            ...
            if not order_id or order_id in order_by_id:
                raise _BacktestFailure(
                    (_error("Orders must have unique non-empty identifiers.", field_path="orders.order_id"),)
                )
            order_by_id[order_id] = order
            symbol = cast(str | None, _field(order, "symbol"))
            signal = _date_value(_field(order, "signal_session"), "order.signal_session")
            execution = _date_value(_field(order, "execution_session"), "order.execution_session")
            quantity = _quantity(_field(order, "requested_quantity"), "order.requested_quantity")
            if quantity == 0 or signal is None or execution is None or execution <= signal:
                raise _BacktestFailure(
                    (_error("An order was not a non-zero whole-share next-session order.", field_path="orders", session=execution, symbol=symbol),)
                )
            ordinal = _quantity(_field(order, "ordinal"), "order.ordinal")
            order_key = (signal, execution, ordinal, symbol or "")
            if previous_order_key is not None and order_key < previous_order_key:
                raise _BacktestFailure(
                    (_error("Orders are not in deterministic signal/execution chronology.", field_path="orders", session=signal, symbol=symbol),)
                )
            ...
            if status in {
                ...
                raise _BacktestFailure(
                    (_error("An unfilled order did not preserve an actionable reason.", field_path="orders.unfilled_reason", session=execution, symbol=symbol),)
                )
        ...
        for fill in fills:
            ...
            if not fill_id or fill_id in fill_ids:
                raise _BacktestFailure(
                    (_error("Fills must have unique non-empty identifiers.", field_path="fills.fill_id"),)
                )
            fill_ids.add(fill_id)
            order = order_by_id.get(order_id)
            if order is None:
                raise _BacktestFailure(
                    (_error("A fill referenced no platform order.", field_path="fills.order_id"),)
                )
            fill_session = _date_value(_field(fill, "session"), "fill.session")
            execution = _date_value(_field(order, "execution_session"), "order.execution_session")
            if fill_session is None or execution != fill_session:
                raise _BacktestFailure(
                    (_error("A fill occurred outside its order execution session.", field_path="fills.session", session=fill_session, symbol=cast(str | None, _field(fill, "symbol"))),)
                )
            quantity = _quantity(_field(fill, "quantity"), "fill.quantity")
            requested = _quantity(_field(order, "requested_quantity"), "order.requested_quantity")
            if quantity == 0 or (quantity > 0) != (requested > 0) or abs(quantity) > abs(requested):
                raise _BacktestFailure(
                    (_error("A fill quantity violated whole-share order direction or requested-size bounds.", field_path="fills.quantity", session=fill_session, symbol=cast(str | None, _field(fill, "symbol"))),)
                )
            ...
            if previous_fill_key is not None and fill_key < previous_fill_key:
                raise _BacktestFailure(
                    (_error("Fills are not in deterministic action chronology.", field_path="fills", session=fill_session, symbol=cast(str | None, _field(fill, "symbol"))),)
                )
            previous_fill_key = fill_key
            if cast(str | None, _field(fill, "symbol")) != cast(str | None, _field(order, "symbol")):
                raise _BacktestFailure(
                    (_error("A fill symbol did not match its order symbol.", field_path="fills.symbol", session=fill_session),)
                )
            filled_by_order[order_id] = filled_by_order.get(order_id, 0) + quantity
            _audit_fill_costs(fill, context)
            if fill_session not in {
                _date_value(_field(state, "session"), "portfolio.session") for state in states
            }:
                raise _BacktestFailure(
                    (_error("A fill had no portfolio state after the fill.", field_path="portfolio_states", session=fill_session),)
                )
        ...
        for order_id, order in order_by_id.items():
            ...
            if abs(filled) > abs(requested):
                raise _BacktestFailure(
                    (_error("Cumulative fills exceeded the requested order quantity.", field_path="fills.quantity"),)
                )
            expected_status = (
                OrderStatus.FILLED
                if abs(filled) == abs(requested)
            ...
            if status is not OrderStatus.PENDING and status is not expected_status:
                raise _BacktestFailure(
                    (_error("Order status did not reconcile to its fills.", field_path="orders.status", symbol=cast(str | None, _field(order, "symbol"))),)
                )
        ...
        for daily_return in returns:
            session = _date_value(_field(daily_return, "session"), "daily_return.session")
            value = _decimal(_field(daily_return, ("return_value", "returns", "value")), "daily_return.return_value")
            if session is None or session not in state_session_set or (previous_return_session is not None and session <= previous_return_session) or value <= Decimal("-1"):
                raise _BacktestFailure(
                    (_error("Daily returns did not reconcile to finite chronological portfolio marks.", field_path="daily_returns", session=session),)
                )
        ...
        return Err(
            (
                _error(
                    str(failure).splitlines()[0] or "The engine output failed accounting audit.",
...
def _configuration_checksum(config: object) -> str:
    ...
    try:
        ...
        dumped = config.model_dump(mode="python") if hasattr(config, "model_dump") else repr(config)
...
def _request_value(
    ...
    *,
    ...
    if isinstance(request, BacktestRequest):
        ...
        if evaluation_range is not None and evaluation_range != request.evaluation_range:
    ...
    if isinstance(request, str):
        ...
        if candidate is not None and requested_range is None:
            if isinstance(candidate, DateRange):
                ...
                start = _date_value(_field(candidate, "start"), "evaluation_range.start")
...
def _manifest(snapshot: object) -> object | None:
    ...
    return candidate if candidate is not None else getattr(snapshot, "snapshot_manifest", None)
...
def _disclosure(snapshot: object) -> LimitationDisclosure:
    ...
    return candidate if isinstance(candidate, LimitationDisclosure) else LimitationDisclosure.current()
...
class BacktestService:
    ...
    def __init__(
        ...
        **compatibility: object,
        ...
        self.tracker = tracker or experiment_tracker or compatibility.pop("tracker_port", None)
        self.snapshot_manager = snapshot_manager or compatibility.pop("snapshot_verifier", None)
        self.bundle_adapter = bundle_adapter or compatibility.pop("bundle", None) or compatibility.pop("bundle_materializer", None)
        self.engine = engine or compatibility.pop("backtest_engine", None)
        self.evaluator = evaluator or evaluation or compatibility.pop("evaluation_service", None)
        self.clock: BacktestClock = cast(
            BacktestClock, clock or compatibility.pop("backtest_clock", None) or _SystemBacktestClock()
        )
        if compatibility:
            unknown = ", ".join(sorted(compatibility))
            raise TypeError(f"unsupported BacktestService arguments: {unknown}")
        if self.tracker is None or self.snapshot_manager is None or self.bundle_adapter is None or self.engine is None:
            raise TypeError("tracker, snapshot_manager, bundle_adapter, and engine are required")
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            return Err((self._input_error(str(failure), field_path="request"),), preserve_order=True)
        ...
        try:
            ...
            if not isinstance(config, ResolvedConfig):
                raise _BacktestFailure((self._input_error("a validated ResolvedConfig is required", field_path="config"),))
            requested_range = normalized_request.evaluation_range or _range_from_config(config)
            snapshot = self._open_snapshot(normalized_request.snapshot_id)
            self._verify_snapshot(snapshot, normalized_request.snapshot_id, requested_range)
            locator = self._materialize_bundle(snapshot, normalized_request.snapshot_id)
            engine_request = BacktestRequest(normalized_request.snapshot_id, requested_range)
            engine_method = self._first_method(self.engine, ("run", "execute", "run_backtest"))
            ...
            audit_result = audit_core_output(output, config, evaluation_range=requested_range)
            ...
            primary_errors = (
                ActionableError.from_unexpected_exception(
                    self.operation_name,
                    failure,
                    correlation_id=str(run_identifier) if run_identifier is not None else None,
                ),
            )
    ...
    def _allocate_run(self, request: BacktestRequest, config: object | None) -> object:
        ...
        universe = getattr(getattr(config, "data", config), "universe", ()) if config is not None else ()
        strategy = getattr(getattr(config, "strategy", config), "identifier", "monthly_momentum_v1") if config is not None else "monthly_momentum_v1"
        values: dict[str, object] = {
            ...
            "config": config,
            "resolved_config": config,
            "configuration_checksum": _configuration_checksum(config) if config is not None else sha256_canonical_json({}),
            "environment_checksum": sha256_canonical_json({"deterministic_seed": getattr(getattr(config, "runtime", None), "deterministic_seed", 0)}),
            ...
            "started_at": now,
            "state": "running",
            "deterministic_seed": getattr(getattr(config, "runtime", None), "deterministic_seed", 0),
        }
        try:
    ...
    def _open_snapshot(self, snapshot_id: str) -> object:
        opener = getattr(self.snapshot_manager, "open_verified", None) or getattr(self.snapshot_manager, "open_snapshot", None)
        if not callable(opener):
            raise _BacktestFailure((self._port_error("snapshot.open", "snapshot verifier has no open_verified method"),))
        return _unwrap(_invoke(opener, positional=(snapshot_id,), values={"snapshot_id": snapshot_id}), "snapshot.open")

    def _verify_snapshot(self, snapshot: object, snapshot_id: str, requested_range: DateRange) -> None:
        if getattr(snapshot, "snapshot_id", None) != snapshot_id:
            raise _BacktestFailure((self._port_error("snapshot.open", "verified snapshot changed its Snapshot_ID", field_path="snapshot_id"),))
        available, comparison_ready = _readiness(snapshot)
        if available is False:
            raise _BacktestFailure((self._port_error("snapshot.readiness", "the selected snapshot is unavailable", field_path="snapshot.availability"),))
        if comparison_ready is False:
            raise _BacktestFailure((self._port_error("snapshot.readiness", "the selected snapshot is not ready for benchmark evaluation", field_path="snapshot.comparison_ready"),))
        covered = _snapshot_range(snapshot)
        if covered is not None and (requested_range.start < covered.start or requested_range.end > covered.end):
            raise _BacktestFailure((self._port_error("snapshot.range", "evaluation range is outside the pinned snapshot range", field_path="evaluation_range"),))
        inspector = getattr(self.snapshot_manager, "inspect_snapshot", None)
        if (available is None or comparison_ready is None or covered is None) and callable(inspector):
            inspected = _unwrap(_invoke(inspector, positional=(snapshot_id,), values={"snapshot_id": snapshot_id}), "snapshot.inspect")
            if getattr(inspected, "snapshot_id", snapshot_id) != snapshot_id:
                raise _BacktestFailure((self._port_error("snapshot.inspect", "snapshot inspection changed the pinned Snapshot_ID", field_path="snapshot_id"),))
            inspected_available, inspected_ready = _readiness(inspected)
            if inspected_available is False or inspected_ready is False:
                raise _BacktestFailure((self._port_error("snapshot.readiness", "the selected snapshot is not ready for backtesting", field_path="snapshot.readiness"),))
            inspected_range = _snapshot_range(inspected)
            if inspected_range is not None and (requested_range.start < inspected_range.start or requested_range.end > inspected_range.end):
                raise _BacktestFailure((self._port_error("snapshot.range", "evaluation range is outside the pinned snapshot range", field_path="evaluation_range"),))

    def _materialize_bundle(self, snapshot: object, snapshot_id: str) -> object:
        method = self._first_method(self.bundle_adapter, ("materialize", "materialize_bundle"))
        value = _unwrap(_invoke(method, positional=(snapshot,), values={"snapshot": snapshot, "snapshot_handle": snapshot, "snapshot_id": snapshot_id}), "zipline.bundle.materialize")
        if getattr(value, "snapshot_id", None) != snapshot_id:
            raise _BacktestFailure((self._port_error("zipline.bundle.materialize", "the exact bundle is not pinned to the selected Snapshot_ID", field_path="bundle.snapshot_id"),))
        bundle_name = getattr(value, "bundle_name", None)
        if isinstance(bundle_name, str) and bundle_name.strip().lower() == "latest":
            raise _BacktestFailure((self._port_error("zipline.bundle.materialize", "mutable latest-bundle resolution is not allowed", field_path="bundle.bundle_name"),))
        return value
    ...
    def _finalize_success(self, result: BacktestResult) -> None:
        ...
        if method is None:
            raise _BacktestFailure((self._port_error("experiment.finalize", "experiment tracker has no success finalization method", field_path="run_id"),))
        values = {
        ...
        try:
            finalized = _invoke(
                method,
                positional=(result.run_id, result),
                values=values,
            )
            ...
            raise _BacktestFailure((ActionableError.from_unexpected_exception("experiment.finalize", failure, correlation_id=str(result.run_id)),)) from None

    def _finalize_failure(self, run_id: object, errors: tuple[ActionableError, ...]) -> tuple[ActionableError, ...]:
        method = self._optional_method(
            self.tracker,
    ...
    def _first_method(target: object, names: Sequence[str]) -> Callable[..., object]:
        ...
        if method is None:
            raise _BacktestFailure((BacktestService._port_error(names[0], f"required port method is unavailable: {', '.join(names)}"),))
        return method

    @staticmethod
    def _optional_method(target: object, names: Sequence[str]) -> Callable[..., object] | None:
        for name in names:
    ...
    @staticmethod
    def _port_error(operation: str, message: str, *, field_path: str | None = None) -> ActionableError:
        return ActionableError(
            operation=operation,
            category=ErrorCategory.EXPERIMENT_RECORDING if operation.startswith("experiment") else ErrorCategory.SNAPSHOT_NOT_READY,
            message=message,
