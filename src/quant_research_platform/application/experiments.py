...
class RunInputs:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            run_id is not None
            and platform_run_id is not None
            and run_id != platform_run_id
        ):
        ...
        if isinstance(evaluation_start, datetime) or not isinstance(
            evaluation_start, date
        ):
...
def _unwrap(value: object, operation: str) -> object:
    ...
    if value is None:
        raise _ExpectedFailure(
            (_error(operation, "The tracking adapter returned no result."),)
        )
    return value
...
def _error(
    ...
    message: str,
    *,
    category: ErrorCategory = ErrorCategory.EXPERIMENT_RECORDING,
...
def _metrics(result: object) -> tuple[EvaluationMetrics, ...]:
    ...
    return tuple(
        sorted(values, key=lambda item: str(getattr(item.scope, "value", item.scope)))
    )
...
def _descriptor(value: object, ordinal: int) -> ArtifactDescriptor | None:
    ...
    uri = str(
        _field(value, ("relative_uri", "uri", "path"), f"artifacts/sha256/{checksum}")
    )
    payload = _field(value, ("payload", "bytes"), None)
    if payload is not None and not isinstance(payload, bytes):
        payload = None
    size = _field(
        value, ("byte_size", "size"), len(payload) if payload is not None else 0
    )
    try:
...
class ExperimentTracker:
    ...
    def create_run(
        self, inputs: RunInputs | Mapping[str, object] | None = None, **values: object
    ) -> Result[RunHandle]:
        ...
        try:
            ...
            tracker_owns_metadata = (
                self.metadata_store is not None
                and _field(self.mlflow_tracker, "metadata_store") is self.metadata_store
            )
            ...
            if self.mlflow_tracker is None:
                if self.metadata_store is None:
                    raise _ExpectedFailure(
                        (
                            _error(
                                "experiment.allocate",
                                "No tracking or metadata adapter was configured.",
                            ),
                        )
                    )
                ...
                allocated = _unwrap(
                    _invoke(method, values=delegated_values), "experiment.allocate"
                )
    ...
    def succeed(self, run: RunHandle | UUID, result: object) -> Result[RunHandle]:
        ...
        try:
            ...
            if not isinstance(disclosure, LimitationDisclosure):
                raise _ExpectedFailure(
                    (
                        _error(
                            "experiment.finalize",
                            "A run result lacks a valid limitation disclosure.",
            ...
            if manifest_checksum is None:
                raise _ExpectedFailure(
                    (
                        _error(
                            "experiment.finalize",
                            "A successful run requires a checksummed Run Manifest.",
                            run_id=handle.run_id,
                        ),
                    )
                )
            scientific_checksums = tuple(
                item.checksum for item in descriptors if item.scientific
            )
                ...
                if existing != (RunState.SUCCEEDED, payload_checksum):
                    raise _ExpectedFailure(
                        (
                            _error(
                                "experiment.finalize",
                                "The terminal run already has a different success payload.",
                                run_id=handle.run_id,
                            ),
                        )
                    )
                return Ok(
                    RunHandle(handle.run_id, handle.mlflow_run_id, RunState.SUCCEEDED)
                )
            ...
            method = self._method(
                self.mlflow_tracker,
                ("finalize_success", "succeed", "record_success", "complete_run"),
            )
            finalized = _unwrap(
                _invoke(
                    method,
                    positional=(handle, result),
                    values={
                        "run": handle,
                        "run_handle": handle,
                        "run_id": handle.run_id,
            ...
            terminal = self._handle_from(
                finalized, handle.run_id, state=RunState.SUCCEEDED
            )
            ...
            return Err(
                (
                    ActionableError.from_unexpected_exception(
                        "experiment.finalize",
                        failure,
                        correlation_id=str(run) if isinstance(run, UUID) else None,
                    ),
                ),
                preserve_order=True,
            )
    ...
    def fail(
        ...
        try:
            ...
            if not safe_errors:
                safe_errors = (
                    _error(
                        ...
                        "The run failed without a structured diagnostic.",
                        run_id=handle.run_id,
                    ),
                )
            descriptors = self._prepare_artifacts_from_values(
                diagnostics, handle.run_id
            )
            payload_checksum = sha256_canonical_json(
                {
                ...
                if existing != (RunState.FAILED, payload_checksum):
                    raise _ExpectedFailure(
                        (
                            _error(
                                "experiment.finalize_failure",
                                "The terminal run already has a different failure payload.",
                                run_id=handle.run_id,
                            ),
                        )
                    )
                return Ok(
                    RunHandle(handle.run_id, handle.mlflow_run_id, RunState.FAILED)
                )
            if handle.state is not RunState.RUNNING:
                raise _ExpectedFailure(
                    (
            ...
            method = self._method(
                self.mlflow_tracker,
                ("finalize_failure", "fail", "record_failure", "fail_run"),
            )
            finalized = _unwrap(
                _invoke(
                    method,
                    positional=(handle, safe_errors),
                    values={
                        "run": handle,
                        "run_handle": handle,
                        "run_id": handle.run_id,
            ...
            terminal = self._handle_from(
                finalized, handle.run_id, state=RunState.FAILED
            )
            ...
            if self.mlflow_tracker is not None:
                ...
                if callable(method):
                    opened = _unwrap(
                        _invoke(
                            method,
                            positional=(identifier, checksum),
                            values={"run_id": identifier, "checksum": checksum},
                        ),
                        "artifact.verify",
                    )
            ...
            return Err(
                (
                    ActionableError(
                        ...
                        correlation_id=str(failed_identifier)
                        if failed_identifier
                        else None,
                    ),
                ),
                preserve_order=True,
    ...
    def _inputs(
        self,
        inputs: RunInputs | Mapping[str, object] | None,
        values: Mapping[str, object],
    ) -> RunInputs:
        ...
        evaluation_range = merged.get("evaluation_range") or _field(
            request, "evaluation_range"
        )
        ...
        return RunInputs(
            run_id=cast(
                UUID | None, merged.get("run_id") or merged.get("platform_run_id")
            ),
            snapshot_id=str(
                merged.get("snapshot_id") or _field(request, "snapshot_id", "")
            ),
            strategy_identifier=str(
                merged.get("strategy_identifier")
                or merged.get("strategy_id")
                or _field(strategy, "identifier", "monthly_momentum_v1")
            ),
            ...
            or merged.get("fingerprint"),
            configuration_checksum=_checksum(
                merged.get("configuration_checksum"), {"configuration": _plain(config)}
            ),
            environment_checksum=_checksum(
                merged.get("environment_checksum"),
                {
                    "environment": _plain(
                        merged.get("environment_fingerprint")
                        or merged.get("fingerprint")
                    )
                },
            ),
            universe=cast(
                Sequence[str], merged.get("universe") or _field(data, "universe", ())
            ),
            deterministic_seed=int(
                cast(
                    int | str,
                    merged.get(
                        "deterministic_seed", _field(runtime, "deterministic_seed", 0)
                    ),
                )
            ),
            secret_values=cast(Sequence[str], merged.get("secret_values", ())),
            limitation_disclosure=cast(
                LimitationDisclosure | None, merged.get("limitation_disclosure")
            ),
        )

    def _tracking_values(
        self, inputs: RunInputs, run_id: UUID, supplied: Mapping[str, object]
    ) -> dict[str, object]:
        ...
        if not callable(method):
            raise _ExpectedFailure(
                (
                    _error(
                        "experiment.allocate",
                        "The metadata adapter cannot create a running run.",
                        run_id=run_id,
                    ),
                )
            )
    ...
    def _prepare_artifacts(
        self, result: object, run_id: UUID
    ) -> tuple[ArtifactDescriptor, ...]:
        return self._prepare_artifacts_from_values(_artifact_candidates(result), run_id)

    def _prepare_artifacts_from_values(
        self, values: Sequence[object], run_id: UUID
    ) -> tuple[ArtifactDescriptor, ...]:
        descriptors = tuple(
            item
            for index, value in enumerate(values)
            if (item := _descriptor(value, index)) is not None
        )
        prepared: list[ArtifactDescriptor] = []
        for descriptor in sorted(
            descriptors, key=lambda item: (item.role, item.checksum)
        ):
            current = descriptor
    ...
    def _publish_artifact(
        self, artifact: ArtifactDescriptor, run_id: UUID
    ) -> ArtifactDescriptor:
        ...
        try:
            ...
            if callable(publish) and callable(create_staging) and callable(stage_bytes):
                staging = _invoke(
                    create_staging,
                    values={
                        "operation_id": f"experiment-{artifact.checksum[:16]}-{uuid4().hex}"
                    },
                )
                staged = _invoke(
                    stage_bytes,
                    positional=(
                        staging,
                        f"runs/{run_id}/{artifact.role}-{artifact.checksum}.bin",
                        artifact.payload,
                    ),
                    values={
                        "staging": staging,
                ...
                return ArtifactDescriptor(
                    ...
                    str(
                        _field(
                            reference, ("relative_uri", "uri"), artifact.relative_uri
                        )
                    ),
                    artifact.byte_size,
                    artifact.media_type,
    ...
    def _open_from_store(self, run_id: UUID, checksum: str) -> VerifiedArtifact:
        if self.artifact_store is None:
            raise _ExpectedFailure(
                (
                    _error(
                        "artifact.verify",
                        "No artifact store was configured.",
                        category=ErrorCategory.INTEGRITY_CHECKSUM,
                        checksum=checksum,
                        run_id=run_id,
                    ),
        ...
        record = (
            self.metadata_store.get_artifact(checksum)
            if self.metadata_store is not None
            else None
        )
        reference = _field(record, ("reference", "artifact_reference"), record)
        method = getattr(
            self.artifact_store, "open_verified_artifact", None
        ) or getattr(self.artifact_store, "stream_artifact", None)
        if not callable(method):
            raise _ExpectedFailure(
                (
                    _error(
                        "artifact.verify",
                        "The artifact store has no verified streaming method.",
                        category=ErrorCategory.INTEGRITY_CHECKSUM,
                        checksum=checksum,
                        run_id=run_id,
                    ),
        ...
        opened = _invoke(
            ...
            values={"reference": reference, "checksum": checksum},
        )
        stream = (
            opened
            if callable(opened)
            else (lambda opened=opened: cast(Iterable[bytes], opened))
        )
        return VerifiedArtifact(
            run_id,
            checksum,
            str(_field(record, "relative_uri", _field(reference, "relative_uri", ""))),
            int(
                cast(
                    int | str,
                    _field(record, "byte_size", _field(reference, "byte_size", 0)),
                )
            ),
            str(_field(record, "media_type", "application/octet-stream")),
            "available",
            cast(Callable[[], Iterable[bytes]], stream),
        )

    def _artifact_from_opened(
        self, run_id: UUID, checksum: str, opened: object
    ) -> VerifiedArtifact:
        ...
        if not callable(stream_method):
            if callable(opened):
                ...
                def stream_method() -> Iterable[bytes]:
        ...
        return VerifiedArtifact(
            run_id,
            checksum,
            str(_field(opened, "relative_uri", _field(record, "relative_uri", ""))),
            int(
                cast(
                    int | str,
                    _field(opened, "byte_size", _field(record, "byte_size", 0)),
                )
            ),
            str(
                _field(
                    opened,
                    "media_type",
                    _field(record, "media_type", "application/octet-stream"),
                )
            ),
            "available",
            cast(Callable[[], Iterable[bytes]], stream_method),
        )
    ...
    def _verify_metadata_available(self, checksum: str) -> None:
        ...
        if str(getattr(availability, "value", availability)) not in {
            "available",
            ...
            raise _ExpectedFailure(
                (
                    _error(
                        "artifact.verify",
                        "The artifact is unavailable or already invalid.",
                        category=ErrorCategory.INTEGRITY_CHECKSUM,
                        checksum=checksum,
                    ),
                )
    ...
    def _assert_run_artifact(self, run_id: UUID, checksum: str) -> None:
        for name in ("list_run_artifacts", "get_run_artifacts", "run_artifacts"):
            ...
            values = cast(
                tuple[object, ...], tuple(cast(Iterable[object], links or ()))
            )
    ...
    def _resolve_handle(self, value: RunHandle | UUID) -> RunHandle:
        ...
        if not isinstance(value, UUID):
            raise _ExpectedFailure(
                (_error("experiment.run", "The run handle is invalid."),)
            )
        ...
        if self.metadata_store is not None:
            ...
            state = RunState(
                cast(str | RunState, _field(record, "state", RunState.RUNNING))
            )
            handle = RunHandle(
                value, cast(str | None, _field(record, "mlflow_run_id")), state
            )
        ...
        raise _ExpectedFailure(
            (
                _error(
                    "experiment.run",
                    "The run is not known to this process.",
                    run_id=value,
                ),
            )
        )
    ...
    @staticmethod
    def _handle_from(
        value: object, run_id: UUID, *, state: RunState = RunState.RUNNING
    ) -> RunHandle:
        if isinstance(value, RunHandle):
            return value
        ...
        return RunHandle(
            identifier,
            cast(str | None, _field(value, ("mlflow_run_id", "mlflow_id"))),
            RunState(cast(str | RunState, selected_state)),
        )
    ...
    def _method(target: object | None, names: Sequence[str]) -> Callable[..., object]:
        if target is None:
            raise _ExpectedFailure(
                (_error("experiment.adapter", "No tracking adapter was configured."),)
            )
        for name in names:
        ...
        raise _ExpectedFailure(
            (
                _error(
                    "experiment.adapter",
                    "The tracking adapter does not implement the required lifecycle method.",
                ),
            )
        )
    ...
    def _manifest(result: object) -> tuple[str | None, str | None]:
        ...
        uri = _field(
            ...
            _field(manifest, ("manifest_uri", "relative_uri")),
        )
        return (
            str(checksum) if isinstance(checksum, str) else None,
            str(uri) if isinstance(uri, str) else None,
        )
