...
- [x] 1. Establish the Prompt Zero Python foundation before engine logic
  - [x] 1.1 Record the Phase 1 technology decision in `docs/decisions/0001-phase1-local-stack.md`
  ...
  - [x] 1.2 Create `pyproject.toml` and the reviewed `uv.lock` workflow for Python 3.11
  ...
  - [x] 1.3 Create the Phase 1 source, test, configuration, and generated-data layout
  ...
  - [x] 1.4 Add project-boundary, build, import, and architecture smoke tests
...
- [x] 2. Checkpoint — Verify the Prompt Zero foundation
  - Ensure all tests pass, ask the user if questions arise.

- [x] 3. Implement canonical domain primitives and scientific identity rules
  - [x] 3.1 Implement errors, results, disclosures, and stable reason taxonomies in `domain/errors.py`
  ...
  - [x] 3.2 Implement canonical encoding and checksum helpers in `domain/canonical.py`
  ...
  - [x] 3.3 Implement market, manifest, and storage-reference value objects in `domain/market.py` and `domain/manifests.py`
  ...
  - [x] 3.4 Implement strategy, execution, evaluation, run, and job value objects in `domain/strategy.py`, `domain/execution.py`, and `domain/evaluation.py`
  ...
  - [x] 3.5 Add domain primitive and canonical-manifest unit tests
...
- [x] 4. Implement validated configuration, project-root resolution, and redaction
  - [x] 4.1 Implement frozen Pydantic v2 configuration models in `config/models.py`
  ...
  - [x] 4.2 Implement safe loading, precedence, and path boundaries in `config/loader.py` and `config/project_root.py`
  ...
  - [x] 4.3 Implement canonical non-secret serialization and the reusable redactor in `config/serializer.py`
  ...
  - [x] 4.4 Write the Hypothesis test for Property 1 in `tests/properties/test_property_01_configuration_resolution.py`
  ...
  - [x] 4.5 Write the Hypothesis test for Property 2 in `tests/properties/test_property_02_configuration_round_trip.py`
  ...
  - [x] 4.6 Add configuration parser, precedence, bounds, path, and redaction example tests
...
- [x] 5. Implement XNYS sessions and the narrow yfinance provider boundary
  - [x] 5.1 Implement `ExchangeCalendar` and the pinned XNYS adapter in `infrastructure/xnys_calendar.py`
  ...
  - [x] 5.2 Define `MarketDataProvider`, request/result contracts, batching, and retry policy helpers in `application/ports.py`
  ...
  - [x] 5.3 Implement the yfinance adapter in `infrastructure/yfinance_provider.py`
  ...
  - [x] 5.4 Write the Hypothesis test for Property 3 in `tests/properties/test_property_03_provider_orchestration.py`
  ...
  - [x] 5.5 Add yfinance and XNYS contract tests with local golden fixtures
...
- [x] 6. Implement deterministic Parquet, DuckDB metadata, content-addressed storage, and jobs
  - [x] 6.1 Define explicit Arrow schemas and canonical table conversion in `infrastructure/schemas.py`
  ...
  - [x] 6.2 Implement bounded deterministic Parquet writes and projected scans in `infrastructure/parquet_store.py`
  ...
  - [x] 6.3 Implement the DuckDB schema and repositories in `infrastructure/duckdb_metadata.py`
  ...
  - [x] 6.4 Implement staging, CAS artifacts, integrity checks, and the publisher lock in `infrastructure/filesystem_store.py`
  ...
  - [x] 6.5 Implement synchronous jobs and sanitized structured logging in `application/jobs.py` and `infrastructure/logging.py`
  ...
  - [x] 6.6 Add DuckDB/PyArrow/filesystem/job contract tests
...
- [x] 7. Implement causal normalization, corporate actions, validation, quarantine, gaps, and staleness
  - [x] 7.1 Implement `CausalForwardAdjustmentV1` and `Normalizer` in `domain/normalization.py`
  ...
  - [x] 7.2 Write the Hypothesis test for Property 4 in `tests/properties/test_property_04_normalization_confluence.py`
  ...
  - [x] 7.3 Implement streaming deterministic validation in `domain/validation.py`
  ...
  - [x] 7.4 Write the Hypothesis test for Property 5 in `tests/properties/test_property_05_validation_partition.py`
  ...
  - [x] 7.5 Create reviewed action and quality golden fixtures
  ...
  - [x] 7.6 Add normalization-to-Parquet integration tests
...
- [x] 8. Implement immutable, atomic, idempotent, and incremental snapshot ingestion
  - [x] 8.1 Implement snapshot identity/manifest assembly in `application/snapshots.py`
  ...
  - [x] 8.2 Implement verified snapshot open/list and immutability guards in `application/snapshots.py`
  ...
  - [x] 8.3 Implement atomic publication and startup reconciliation in `infrastructure/filesystem_store.py`
  ...
  - [x] 8.4 Implement revision-overlap incremental merging in `application/incremental.py`
  ...
  - [x] 8.5 Implement staged `DataIngestionService` orchestration in `application/ingestion.py`
  ...
  - [x] 8.6 Write the Hypothesis test for Property 6 in `tests/properties/test_property_06_snapshot_identity.py`
  ...
  - [x] 8.7 Write the Hypothesis test for Property 7 in `tests/properties/test_property_07_publication_state_machine.py`
  ...
  - [x] 8.8 Add snapshot/ingestion fault-injection and incremental integration tests
...
- [x] 9. Checkpoint — Verify the reproducible data plane
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. Implement the verified snapshot-to-Zipline bundle adapter
  - [x] 10.1 Implement `ZiplineBundleAdapter` in `infrastructure/zipline_bundle.py`
  ...
  - [x] 10.2 Add the pinned Zipline bundle contract test in `tests/contract/test_zipline_bundle_contract.py`
  ...
  - [x] 10.3 Add split/dividend golden bundle fixtures and exactly-once action tests
  ...
  - [x] 10.4 Add derived-bundle rebuild and projection integration tests
...
- [x] 11. Implement complete and deterministic monthly momentum decisions
  - [x] 11.1 Implement `monthly_momentum_v1` in `domain/strategy.py`
  ...
  - [x] 11.2 Implement causal decision delivery and order-intent generation in `application/decisions.py`
  ...
  - [x] 11.3 Write the Hypothesis test for Property 8 in `tests/properties/test_property_08_momentum_decisions.py`
  ...
  - [x] 11.4 Add momentum boundary and decision-artifact golden tests
...
- [x] 12. Implement next-session Zipline execution, accounting, and backtest orchestration
  - [x] 12.1 Implement `CashSafeOpenBlotter` in `infrastructure/zipline_engine.py`
  ...
  - [x] 12.2 Implement the Zipline `BacktestEngine` adapter in `infrastructure/zipline_engine.py`
  ...
  - [x] 12.3 Implement accounting audits and `BacktestService` in `application/backtests.py`
  ...
  - [x] 12.4 Write the Hypothesis test for Property 9 in `tests/properties/test_property_09_no_look_ahead.py`
  ...
  - [x] 12.5 Write the Hypothesis test for Property 10 in `tests/properties/test_property_10_execution_accounting.py`
  ...
  - [x] 12.6 Add pinned Zipline execution and overnight-gap golden contract tests
...
- [x] 13. Implement SPY-aligned deterministic evaluation and artifacts
  - [x] 13.1 Implement metric/reference functions in `domain/evaluation.py`
  ...
  - [x] 13.2 Implement `EvaluationService` and canonical result artifacts in `application/evaluation.py`
  ...
  - [x] 13.3 Write the Hypothesis test for Property 11 in `tests/properties/test_property_11_evaluation.py`
  ...
  - [x] 13.4 Add metric, artifact, and SPY-gap golden tests
...
- [x] 14. Implement immutable local MLflow run lifecycle and artifact tracking
  - [x] 14.1 Implement deterministic environment/source fingerprinting in `infrastructure/fingerprint.py`
  ...
  - [x] 14.2 Implement the local MLflow adapter in `infrastructure/mlflow_tracker.py`
  ...
  - [x] 14.3 Implement run finalization intents and terminal repository guards in `infrastructure/duckdb_metadata.py`
  ...
  - [x] 14.4 Implement `ExperimentTracker` orchestration and verified run-artifact access in `application/experiments.py`
  ...
  - [x] 14.5 Add MLflow/DuckDB finalization contract and fault-injection tests
...
- [x] 15. Checkpoint — Verify the complete research engine and immutable run lifecycle
  - Ensure all tests pass, ask the user if questions arise.

- [x] 16. Implement run discovery, comparison, inspection, pagination, and the application facade
  - [x] 16.1 Implement indexed run discovery in `infrastructure/duckdb_metadata.py`
  ...
  - [x] 16.2 Implement `ComparisonService` in `application/comparisons.py`
  ...
  - [x] 16.3 Write the Hypothesis test for Property 13 in `tests/properties/test_property_13_comparison.py`
  ...
  - [x] 16.4 Implement the typed `ResearchApplication` facade and configuration-handle registry in `application/services.py`
  ...
  - [x] 16.5 Implement snapshot/run/artifact inspection and bounded paging in `application/inspection.py`
  ...
  - [x] 16.6 Write the Hypothesis test for Property 15 in `tests/properties/test_property_15_pagination.py`
  ...
  - [x] 16.7 Write the Hypothesis test for Property 14 in `tests/properties/test_property_14_secret_sink_safety.py`
  ...
  - [x] 16.8 Add application-facade, jobs, inspection, and comparison integration tests
...
- [x] 17. Implement the Streamlit end-to-end visual workflow
  - [x] 17.1 Implement redacted presenters and shared Streamlit view components in `ui/presenters.py` and `ui/components.py`
  ...
  - [x] 17.2 Implement `ui/app.py` composition/navigation and Configure/Ingest pages
  ...
  - [x] 17.3 Implement the Snapshots page in `ui/pages/snapshots.py`
  ...
  - [x] 17.4 Implement Backtest and Runs pages in `ui/pages/backtest.py` and `ui/pages/runs.py`
  ...
  - [x] 17.5 Implement the Compare page in `ui/pages/compare.py`
  ...
  - [x] 17.6 Add Streamlit AppTest coverage for the complete visual workflow
...
- [x] 18. Checkpoint — Verify application services and Streamlit workflow
  - Ensure all tests pass, ask the user if questions arise.

- [x] 19. Complete integrated reproducibility, memory, documentation, and release validation
  - [x] 19.1 Add the local full-pipeline integration fixture and test in `tests/integration/test_phase1_pipeline.py`
  ...
  - [x] 19.2 Write the Hypothesis test for Property 12 in `tests/properties/test_property_12_stable_rerun.py`
  ...
  - [x] 19.3 Add bounded-memory behavior tests and a marked local streaming benchmark
  ...
  - [x] 19.4 Add local smoke tests and the explicitly gated external-provider smoke seam
  ...
  - [x] 19.5 Finalize developer and research-operation guidance in `README.md` and `docs/`
  ...
  - [x] 19.6 Add a single-shot local release-validation entry point in `scripts/validate_release.py`
  ...
  - [x] 19.7 Add the final end-to-end Phase 1 acceptance test in `tests/acceptance/test_phase1_vertical_slice.py`
...
- [x] 20. Final checkpoint — Validate the first usable Phase 1 vertical slice
  - Ensure all tests pass, ask the user if questions arise.
